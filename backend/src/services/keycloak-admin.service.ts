...
export class KeycloakAdminService {
  ...
  async createOrGetUser(input: CreateOrGetUserInput) {
    ...
    if (existingByUsername) {
      return { id: existingByUsername.id as string, created: false, user: existingByUsername };
    }

    const existingByEmail = await this.findUserByEmail(input.email);
    if (existingByEmail) {
      return { id: existingByEmail.id as string, created: false, user: existingByEmail };
    }
    ...
    try {
      ...
      if (!id) {
        ...
        if (!createdUser?.id) {
          throw new Error('No se pudo resolver el ID del usuario recién creado en Keycloak.');
        }
